...
export class ApiController {
	...
	async get_feed(@Request() req, @Response() res) {
		...
		const wf = new Brackets(wf => {
			wf.where('follows.user.id = :logged_user_id', {
				logged_user_id: req.user.id,
			});
			wf.orWhere('tweet.user.id = :logged_user_id', {
				logged_user_id: req.user.id,
			});
		});
		...
		if (req.query.bottom_tweet_id) {
			...
			result = await this.tweetRepo
				.createQueryBuilder('tweet')
				.leftJoinAndSelect(
					Follows,
					'follows',
					'follows.followed = tweet.user',
				)
	...
	async post_feed(
		...
		let saved = await this.tweetRepo.save({
	...
	async getOne(@Request() req, @Response() res, @Param('id') id: number) {
		let tweet = await this.tweetRepo.findOne(id, {
			relations: ['faved_by'],
		});
	...
	async tweet_fav(@Request() req, @Response() res, @Param('id') id: number) {
		...
		const tweet: Tweet = { id: id } as Tweet;

		const tweet_exist: boolean = this.tweetRepo.hasId({ id: id } as Tweet);
		if (!tweet_exist) {
			return res.status(422).send();
		}
	...
	async user_faved(
		@Request() req,
		@Response() res,
		@Param('tweet_id') tweet_id: number,
	) {
		const result = await this.favsRepo.find({
			user: req.user as User,
			tweet: { id: tweet_id } as Tweet,
		});
		const data = { faved: result[0] ? true : false };
	...
	async follow(@Request() req, @Response() res, @Param('id') id: number) {
		const exist: boolean = await this.userRepo.hasId({ id: id } as User);
	...
	@UseGuards(AuthenticatedGuard)
	async user_tweets_faved(
		@Request() req,
		@Response() res,
		@Param('id') id: number,
	) {
		const favs: Favs[] = await this.favsRepo.find({
			relations: ['tweet'],
			where: {
				user: { id: id } as User,
			},
		...
		const tweets: Tweet[] = favs.map(item => item.tweet);
